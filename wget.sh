...
# To download the historic, not updated anymore data, use the following url 
#wget -r -nH -g on -A .fna ftp://ftp.ncbi.nlm.nih.gov/genomes/archive/old_refseq/Bacteria/
...
awk '{FS="\t"} !/^#/ {if($12=="Complete Genome") print $20}' assembly_summary.txt | sed -r 's|(ftp://ftp.ncbi.nlm.nih.gov/genomes/all/)(GCF_.+)|\1\2/\2_genomic.fna.gz|' > bacteria-fna-urls.txt
